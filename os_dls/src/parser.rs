...
impl DlsParser {
    ...
    pub fn extract_samples(&self) -> Result<Vec<Sample>, String> {
        ...
        let root_chunk =
            Chunk::read(&mut cursor, 0).map_err(|e| format!("Failed to read root chunk: {}", e))?;

        // Verify it's a DLS file
        let dls_type = root_chunk
            .read_type(&mut cursor)
            .map_err(|e| format!("Failed to read DLS type: {}", e))?;

        if dls_type.value != *b"DLS " {
            return Err(format!(
                "Not a DLS file, found type: {:?}",
                String::from_utf8_lossy(&dls_type.value)
            ));
        }

        // Collect all chunks first to avoid borrow checker issues
        let root_children: Vec<Chunk> = root_chunk
            .iter(&mut cursor)
        ...
        for chunk in &root_children {
            ...
            if chunk.id() == riff::LIST_ID {
                ...
                if list_type.value == *b"wvpl" {
                    ...
                    let wave_chunks: Vec<Chunk> = chunk
                        .iter(&mut cursor)
                        .collect::<Result<Vec<_>, _>>()
    ...
    fn parse_wave_chunk(
        ...
        let subchunks: Vec<Chunk> = wave_chunk
            .iter(cursor)
            .collect::<Result<Vec<_>, _>>()
        ...
        for subchunk in &subchunks {
            match subchunk.id() {
                ...
                id if id == riff::LIST_ID => {
                    ...
                    let list_type = subchunk
                        .read_type(cursor)
                        .map_err(|e| format!("Failed to read LIST type: {}", e))?;

                    if list_type == INFO_ID {
                        // Collect INFO chunks
                        let info_chunks: Vec<Chunk> = subchunk
                            .iter(cursor)
                            .collect::<Result<Vec<_>, _>>()
                        ...
                        for info_chunk in &info_chunks {
                            if info_chunk.id() == INAM_ID {
                                let name_data = info_chunk
                                    .read_contents(cursor)
                                    .map_err(|e| format!("Failed to read name: {}", e))?;

                                // Remove null terminator
                                let name_str = if let Some(null_pos) =
                                    name_data.iter().position(|&b| b == 0)
                                {
                                    &name_data[..null_pos]
                                } else {
                                    &name_data
