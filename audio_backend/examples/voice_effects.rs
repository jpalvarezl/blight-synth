...
pub fn main() {
    match &mut BlightAudio::new() {
        Ok(audio) => {
            ...
            audio.send_command(Command::AddVoiceEffect { voice_id: 0, effect: audio.get_effect_factory().create_mono_gain(1.0) });
            ...
            audio.send_command(Command::AddVoiceEffect { voice_id: 1, effect: audio.get_effect_factory().create_mono_gain(0.1) });
            ...
            audio.send_command(Command::AddVoiceEffect { voice_id: 2, effect: audio.get_effect_factory().create_mono_gain(0.5) });
            thread::sleep(std::time::Duration::from_millis(500));
            ...
            audio.send_command(Command::AddVoiceEffect { voice_id: 3, effect: audio.get_effect_factory().create_mono_gain(1.0) });
            thread::sleep(std::time::Duration::from_millis(500));
