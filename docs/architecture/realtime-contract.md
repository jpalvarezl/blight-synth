...
- #182 gives the standalone host a dedicated NRT control worker: Tokio enqueues bounded requests without blocking, while the worker retains the FIFO front command across RT-ring `Full` responses and emits protocol success only after ring acceptance. The tracker remains on transitional nonblocking submission until #181 provides equivalent NRT ownership; #173 stays open until then.
