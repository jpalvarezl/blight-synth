...
The `engine` crate now owns generic instrument dispatch, planar mixing, and master effects. `audio_backend` still combines CPAL, `AudioProcessor`, tracker `Player`/track state, command transport, OSC, resources, and project hydration. The remaining M0/M1 issues linked in frontmatter own command, host, composition, and semantic separation. Until they land, code and tests describe current behavior.
