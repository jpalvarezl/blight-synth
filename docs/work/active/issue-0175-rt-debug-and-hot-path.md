...
- Shared `dsp::rt_{debug,info,warn,error}_log!` severity macros.
...
- [x] `cargo test --workspace --all-targets` — 56 tests
...
- Completed: severity-preserving macros, callback log migration, release compile-out behavior, static checker/CI, and full local/default/host-free/release validation.
